...
@WebServlet("/gearquery")   // Configure the request URL for this servlet (Tomcat 7/Servlet 3.0 upwards)
public class GearQueryServlet extends HttpServlet {
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try (
         ...
         String[] brand = request.getParameterValues("brand");  // Returns an array of Strings
         String[] type = request.getParameterValues("type");  // Returns an array of Strings
         
         String sqlStr = "SELECT * FROM gear WHERE brand IN ("; //SEARCH brand
         for (int i = 0; i < brand.length; ++i) {
            if (i < brand.length - 1) {
               sqlStr += "'" + brand[i] + "', ";  // need a commas
            } else {
               sqlStr += "'" + brand[i] + "'";    // no commas
            }
         }
         
         sqlStr += ") AND type IN ("; //SEARCH type
         for (int i = 0; i < type.length; ++i) {
            if (i < type.length - 1) {
               sqlStr += "'" + type[i] + "', ";  // need a commas
            } else {
               sqlStr += "'" + type[i] + "'";    // no commas
            }
         }
         ...
         out.println("</div>");
         out.println("<p style='align-content: right'><input type='submit' value='Add to Cart' /></p>");
         out.println("</form>");
         out.println("</div>");
